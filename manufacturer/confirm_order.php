	...
	session_start();
		if(isset($_SESSION['manufacturer_login'])) {
			...
			while($rowAvailQuantity = mysqli_fetch_array($resultAvailQuantity)){
				$availProId=array($rowAvailQuantity['pro_id']);
				$availQuantity= array($rowAvailQuantity['quantity']);
			...
			while($rowOrderQuantity = mysqli_fetch_array($resultOrderQuantity)){
				$orderProId =array($rowOrderQuantity['p']);
				$orderQuantity =array( $rowOrderQuantity['q']);
