	...
	if(isset($_SESSION['manufacturer_login'])) {
		...
		if($_SERVER['REQUEST_METHOD'] == "POST") {
			if(!empty($_POST['txtOldPassword'])){
				...
				$query_oldPassword = "SELECT password FROM manufacturer WHERE username= '" . $_SESSION["sessUsername"] . "'";
				...
				if(password_verify($password,$row_oldPassword["password"])) {
					if(!empty($_POST['txtNewPassword']) && !empty($_POST['txtConfirmPassword'])){
						...
						if(strcmp($newPassword,$confirmPassword) == 0) {
							...
							$query_UpdatePassword = "UPDATE manufacturer SET password='$confirmPassword' WHERE username='" . $_SESSION["sessUsername"] . "'";
