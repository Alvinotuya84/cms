	...
	if(isset($_SESSION['retailer_login'])) {
		...
		if($_SERVER['REQUEST_METHOD'] == "POST") {
			if(!empty($_POST['txtOldPassword'])){
				...
				$query_oldPassword = "SELECT password FROM retailer WHERE retailer_id='$id' AND username= '" . $_SESSION["sessUsername"] . "'";
				...
				if(password_verify($password,$row_oldPassword["password"])) {
					if(!empty($_POST['txtNewPassword']) && !empty($_POST['txtConfirmPassword'])){
						...
						if(strcmp($newPassword,$confirmPassword) == 0) {
							$password_stmt=password_hash($confirmPassword, PASSWORD_DEFAULT);
							$query_UpdatePassword = "UPDATE retailer SET password='$password_stmt' WHERE retailer_id='$id'";
